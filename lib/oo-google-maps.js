...
Template.ooGoogleMaps.events({
  ...
  'click .js-OpenGoogleMap': function(e) {
    ...
    if (self.address) {
      ...
      href = "http://maps.google.com/?q=" + addressPlus;
      console.log('%c href   ',  'background: #B3CC57; color: white; padding: 1px 15px 1px 5px;', href);
    } else
      href = "http://maps.google.com/?q=" + self.lat + "," + self.lng;

    if(typeof device === "undefined"){
         if (!Session.equals("isTouch", true)) {
           var ref = window.open(href, '_blank');
         }
       } else {
           var ref = window.open(href, '_blank', 'location=yes', 'closebuttoncaption=Back');
       }
  }
});
